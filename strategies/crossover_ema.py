...
def backtest_crossover_ema(asset, price_col='price',
    ...
    df['long_exit'] = (df['ema_fast'] < df['ema_slow']) & (prev_fast >= prev_slow)
...
if __name__ == "__main__":
    ...
    asset = load_asset()
    results = backtest_crossover_ema(asset)

    print("Buy & Hold:", results['cum_asset'].iloc[-1])
    print("Crossover EMA Strategy:", results['cum_strategy'].iloc[-1])
    print(results.tail())
    print(results[['price','ema_fast','ema_slow']].head(15))